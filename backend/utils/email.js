...
export async function sendApplicationEmail(applicationData, loginCredentials) {
  try {
    ...
    // Login URL
    const loginUrl = process.env.FRONTEND_URL || process.env.VITE_FRONTEND_URL || "http://frontend.jssabhiyan.in/";

    // Email HTML template - Exact format as requested
    const htmlContent = `
<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <p><strong>Application Number:</strong> ${applicationData.applicationNumber || "N/A"}</p>
  <p><strong>Username / Login ID:</strong> ${loginCredentials.identifier}</p>
  <p><strong>Password:</strong> ${loginCredentials.password}</p>
  
  <p>You can log in to your applicant dashboard using the link below:</p>
  ...
  <p>Best Regards,<br>Support Team<br>Jan Swasthya Sahayata Abhiyan</p>
</body>
</html>
    `;
