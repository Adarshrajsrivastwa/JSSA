...
/**
 * Generate HTML for application PDF matching ApplicationSlip component
 */
async function generateApplicationPdfHtml(applicationData, jobPosting, photoSrc, signatureSrc) {
  ...
  const advtNo = jobPosting?.advtNo || "N/A";
  const postTitle = jobPosting?.postTitle?.en || jobPosting?.post?.en || "N/A";
  const applicationNumber = applicationData.applicationNumber || "N/A";
  ...
  return `
    ...
    <body>
      ...
      <div class="logoWrapper">
        <img src="${process.env.FRONTEND_URL || 'https://jssabhiyan.com'}/assets/jss.png" alt="Jan Swasthya Sahayata Abhiyan" class="logo" />
      </div>

      <div class="banner">${postTitle} Recruitment</div>

      <div class="strip">
        <span><strong>Advt. No.:</strong> ${advtNo}</span>
        <span class="stripCenter">Application Slip</span>
        <span><strong>Date:</strong> ${currentDate}</span>
      </div>

      <div class="postRow">
        <span><strong>Post Applied for:</strong> ${postTitle}</span>
        <span><strong>Application No.:</strong> ${applicationNumber}</span>
      </div>

      <div class="sectionHead">&nbsp;Personal Details</div>
        ...
        <table>
          <tbody>
            <tr>
              ...
              <td class="tdValue">${(applicationData.candidateName || "").toUpperCase()}</td>
            </tr>
            <tr>
              <td class="tdLabel">Father's Name:</td>
              <td class="tdValue">${(applicationData.fatherName || "").toUpperCase()}</td>
            </tr>
            <tr>
              <td class="tdLabel">Mother's Name:</td>
              <td class="tdValue">${(applicationData.motherName || "").toUpperCase()}</td>
            </tr>
            <tr>
              <td class="tdLabel">Date of Birth:</td>
              <td class="tdValue">${formatDOB(applicationData.dob)}</td>
            </tr>
            <tr>
              <td class="tdLabel">Gender:</td>
              <td class="tdValue">${applicationData.gender ? applicationData.gender.charAt(0).toUpperCase() + applicationData.gender.slice(1) : ""}</td>
            </tr>
            <tr>
              <td class="tdLabel">Nationality:</td>
              <td class="tdValue">${applicationData.nationality ? applicationData.nationality.charAt(0).toUpperCase() + applicationData.nationality.slice(1) : "Indian"}</td>
            </tr>
            <tr>
              <td class="tdLabel">Category:</td>
              <td class="tdValue">${(applicationData.category || "").toUpperCase()}</td>
            </tr>
            <tr>
              <td class="tdLabel">Aadhar Number:</td>
              <td class="tdValue">${applicationData.aadhar || ""}</td>
            </tr>
            <tr>
              <td class="tdLabel">PAN Number:</td>
              <td class="tdValue">${(applicationData.pan || "").toUpperCase()}</td>
            </tr>
            <tr>
              <td class="tdLabel">Mobile Number:</td>
              <td class="tdValue">${applicationData.mobile || ""}</td>
            </tr>
            <tr>
              <td class="tdLabel">Email ID:</td>
              <td class="tdValue">${applicationData.email || ""}</td>
            </tr>
            <tr>
              <td class="tdLabel">Permanent Address:</td>
              <td class="tdValue">${(applicationData.address || "").toUpperCase()}</td>
            </tr>
            <tr>
              <td class="tdLabel">State:</td>
              <td class="tdValue">${applicationData.state || ""}</td>
            </tr>
          </tbody>
        </table>
        <div class="photoBox">
          ${photoSrc ? `<img src="${photoSrc}" alt="Applicant Photo" />` : ''}
        </div>
      </div>
      ...
      <table style="margin-bottom: 14px;">
        <tbody>
          <tr>
            ...
            <td class="tdValue">${applicationData.pincode || ""}</td>
          </tr>
        </tbody>
      </table>

      <div class="sectionHead">&nbsp;Educational Details</div>

      <table style="margin-bottom: 18px;">
        <tbody>
          <tr>
            <td class="tdLabel">Higher Education:</td>
            <td class="tdValue">${applicationData.higherEducation || ""}</td>
          </tr>
          <tr>
            <td class="tdLabel">Board/University:</td>
            <td class="tdValue">${applicationData.board || ""}</td>
          </tr>
          <tr>
            <td class="tdLabel">Total Marks:</td>
            <td class="tdValue">${applicationData.marks || ""}</td>
          </tr>
          <tr>
            <td class="tdLabel">Marks in Percentage:</td>
            <td class="tdValue">${applicationData.markPercentage || ""}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="checkRow" style="margin-bottom: 24px; line-height: 1.65;">
        <input type="checkbox" checked class="checkbox" />
        <span>
      ...
      <div class="sigWrapper">
        <div class="sigBlock">
          ...
          <div class="sigLabel">Candidate's Signature</div>
        </div>
      </div>
      ...
      <table class="summaryTable">
        ...
        <tbody>
          <tr>
            <td class="td">${applicationNumber}</td>
            <td class="td">${applicationData.email || ""}</td>
            <td class="td">Complete</td>
            <td class="td">${currentDate}</td>
          </tr>
        </tbody>
      </table>

      <div class="footer">
        <span>${process.env.FRONTEND_URL || 'https://jssabhiyan.com'}/fill_application_print?oid=${applicationData._id || applicationData.id || applicationNumber}</span>
        <span>1/1</span>
      </div>
    </body>
    </html>
...
आवेदन करने के लिए धन्यवाद!
    ...
    let pdfBuffer = null;
    const pdfFileName = `Application_Slip_${applicationData.applicationNumber || 'JSSA'}.pdf`;

    if (pdfFilePath) {
      // Load existing PDF if path is provided
      try {
        if (fs.existsSync(pdfFilePath)) {
          pdfBuffer = fs.readFileSync(pdfFilePath);
          console.log("✅ PDF loaded from file:", pdfFilePath);
        } else {
          console.log("⚠️ PDF file not found:", pdfFilePath);
        }
      } catch (fileError) {
        console.error("⚠️ Error reading PDF file:", fileError);
      }
    } else {
      // Generate PDF on-the-fly
      try {
        const pdfHtmlContent = await generateApplicationPdfHtml(applicationData, jobPosting, photoSrc, signatureSrc);
    ...
    if (pdfBuffer) {
      ...
      console.log("⚠️ No PDF attachment (generation failed)");
    }
    ...
    <div class="footer-section">
      ...
      const browser = await puppeteer.launch({
        ...
        args: [
          ...
          '--no-first-run',
          '--no-zygote',
          '--single-process', // This can help with some dependency issues
          '--disable-gpu'
        ],
      });
      console.log("✅ Puppeteer browser launched");
